# THIS FILE DOES NOT CONTAIN MY CODE - THIS WAS TAKEN FROM THE CODE REPO ASSOCIATED WITH ID8 AND CAN BE FOUND (IN
# CONTEXT) HERE: https://github.com/bapanes/AutoSourceID/blob/main/codes/from-cats-to-locnet-input.py

# N.B. I have tidied this code and checked correctness.
...
def get_lb_ps_centered(lb_ps, lb_centre, xsize=128, isdeg=True,
                       is_lat=True):  ##if input angles are in degree use 'isdeg = True'
...
def create_dataset(folder, file="training.csv", prefix="test", n=50, faint="F0", init_con=0):
